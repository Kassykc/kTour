<template>
    <div v-if="show" class="common_sys_alert_modal">
        <!-- 배경 클릭 시 모달이 닫히지 않도록 설정 -->
        <div class="modal_overlay" @click="handleOverlayClick"></div>
        <div class="modal_content" @click.stop>
            <div v-if="isHtml">{{ message }}</div>
            <div v-if="!isHtml" v-html="message"></div>
            <div class="modal_buttons">
                <div @click="handleOk" class="btn ok">{{ okText }}</div>
                <!-- confirm일 때만 cancel 버튼을 추가 -->
                <div v-if="cancelText" @click="handleCancel" class="btn cancel">{{ cancelText }}</div>
            </div>
        </div>
    </div>
</template>
